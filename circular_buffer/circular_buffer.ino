...
int idx = 0;
unsigned long int sendDataPrev;
...
void loop()
{
  ...
  //Assign idx location in each buffer w/ the measured distances
  beac0_buf[idx] = distances_now[0];
  beac1_buf[idx] = distances_now[1];
  beac2_buf[idx] = distances_now[2];
  beac3_buf[idx] = distances_now[3];
  idx++;
  if (idx == 10) {
    idx = 0;
  }
  ...
  if (millis() > sendDataPrev ) {
    bool sentData = false;
    while (!sentData) { //This is just to ensure we send data at least once, it has always finished after the first iteration
      if(Firebase.ready()) {
        ...
        Serial.println("hanging here 0");
        Firebase.RTDB.setTimestamp(&fbdo, currentPath+"/timestamp");
        Firebase.RTDB.setTimestamp(&fbdo, path+"/timestamp");
        Serial.println("hanging here 1");
        Firebase.RTDB.setFloatAsync(&fbdo, currentPath+"/DistanceToBeacon0", avg[0]);
        Firebase.RTDB.setFloatAsync(&fbdo, currentPath+"/DistanceToBeacon1", avg[1]);
        Firebase.RTDB.setFloatAsync(&fbdo, currentPath+"/DistanceToBeacon2", avg[2]);
        Firebase.RTDB.setFloatAsync(&fbdo, currentPath+"/DistanceToBeacon3", avg[3]);
        Serial.println("hanging here 2");
        Firebase.RTDB.setFloatAsync(&fbdo, path+"/DistanceToBeacon0", avg[0]);
        Firebase.RTDB.setFloatAsync(&fbdo, path+"/DistanceToBeacon1", avg[1]);
        Firebase.RTDB.setFloatAsync(&fbdo, path+"/DistanceToBeacon2", avg[2]);
        Firebase.RTDB.setFloatAsync(&fbdo, path+"/DistanceToBeacon3", avg[3]);
        testCount++;
        sentData=true;
        //delay(750);
        //sleep(1);
    ...
    sendDataPrev = millis() + 1000; //update sendDataPrev to be ~1 second from the finish of the last upload.
